...
$$\begin{aligned}
\underset{G}{\textup{min}}\underset{D}{\textup{max}}V(D,G)=\varepsilon_{x\sim p_{data}(x)}[\textup{log}D(x)]+\varepsilon_{z\sim p_{z}(z)}[\textup{log}(1-D(G(z))] \\
x\sim p_{data}(x): x\textup{ is sampled from real data} \;\;\;\; z\sim p_z(z): z\textup{ is sampled from N(0,1)} \\
D(x): \textup{Probability of D(real)} \;\;\;\; D(G(z): \textup{Probability of D(fake)}
\end{aligned}$$

#### Discriminator loss

$$\begin{aligned}
D_{loss}=\frac{1}{m}\sum^m_{i=1}[\textup{log}D_{rnn}(x_i)+\textup{log}(1-D_{rnn}(G_{rnn}(z_i)))] \\
\Leftrightarrow min\frac{1}{m}\sum^m_{i=1}[-\textup{log}D_{rnn}(x_i)-\textup{log}(1-D_{rnn}(G_{rnn}(z_i)))]
\end{aligned}$$

#### Genertor loss

$$\begin{aligned}
G_{loss}=\sum^m_{i=1}[\textup{log}(1-D_{rnn}(G_{rnn}(z_i)))] \\
\Leftrightarrow min\sum^m_{i=1}\textup{log}[-D_{rnn}(G_{rnn}(z_i))]
\end{aligned}$$
...
$$\begin{aligned}
\underset{z^k}{\textup{min}}Er(X^{tes}, G_{rnn}(Z^k))=1-Simi(X^{tes}, G_{rnn}(Z^k)) \\
\textup{Similarity : covariance for simplicity}
\end{aligned}$$
