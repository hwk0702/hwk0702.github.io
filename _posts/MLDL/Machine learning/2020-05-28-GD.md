...
$$\frac{\delta}{\delta\theta_{j}}{MSE}(\theta)=\frac{2}{m}\sum_{i=1}^{m}(\theta^{\tau}\mathbf{x}^{i}-y^{i})x^{i}_{j}$$

- 그레디언트 벡터

$$\triangledown_{\theta}{MSE}= \begin{pmatrix} \frac{\delta}{\delta\theta_{0}}{MSE}(\theta) \\ \frac{\delta}{\delta\theta_{1}}{MSE}(\theta) \\ \vdots \\ \frac{\delta}{\delta\theta_{n}}{MSE}(\theta) \end{pmatrix} = \frac{2}{m}\mathbf{x}^{\tau}(\mathbf{x}\theta-\mathbf{y})$$
