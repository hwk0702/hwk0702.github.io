...
$$\frac{\delta}{\delta\theta_{j}}\textup{MSE}(\theta)=\frac{2}{m}\sum_{i=1}^{m}(\theta^{\tau}\mathbf{x}^{i}-y^{i})x^{i}_{j}$$

- 그레디언트 벡터

$$\triangledown_{\theta}\textup{MSE}= \begin{pmatrix} \frac{\delta}{\delta\theta_{0}}\textup{MSE}(\theta) \\ \frac{\delta}{\delta\theta_{1}}\textup{MSE}(\theta) \\ \vdots \\ \frac{\delta}{\delta\theta_{n}}\textup{MSE}(\theta) \end{pmatrix} = \frac{2}{m}\mathbf{x}^{\tau}(\mathbf{x}\theta-y)$$

- 경사 하강법의 스텝: $${\theta}^{(\mathrm{nextstep})}={\theta}-{\eta}{\triangledown_{\theta}}\textup{MSE}(\theta)$$
