...
- 트리의 노드를 분할할 때 전체 특성 중에서 최선의 특성을 찾는 대신 **무작위로 선택산 특성 후보 중에서 최적의 특성**을 찾는 식
...
6) 예측을 할 때 모든 예측기의 예측을 계한하고 예측기 가중치 $$\alpha_j$$를 더해 예측 결과 만들고, 가중치 합이 가장 큰 클래스가 예측 결과가 된다
