...
(1) $$-\sum_i p_{t|i}log{q_{t|i}}$$을 $$\mathbf{y}_t$$에 대해서 미분한 값을 구하게 되면
...
(2) $$-\sum_j p_{j|t}log{q_{j|t}}$$을 $$\mathbf{y}_t$$에 대해서 미분한 값을 구하게 되면
...
&=-\sum_{i \neq t}\sum_{j \neq t}p_{i|j}\cdot\frac{1}{q_{i|j}}\cdot2(\mathbf{y}_t-\mathbf{y}_j)\cdot q_{i|j}\cdot q_{t|j} \\
